...
# Chunk audio files longer than 4 seconds into separate files
  ...
  for filename in os.listdir(subdirectory):
    ...
    if "Unidentified" not in subdirectory and ".ipynb_checkpoints" not in filename:
      ...
      with contextlib.closing(wave.open(os.path.join(subdirectory, filename),'r')) as f:
        ...
        # Chunk the audio into 4 second chunks
        if duration > 4:
          myaudio = AudioSegment.from_file(os.path.join(subdirectory, filename), "wav") 
          chunk_length_ms = 4000 # pydub calculates in millisec
          chunks = make_chunks(myaudio, chunk_length_ms) # Make chunks of 4 seconds
          for i, chunk in enumerate(chunks):
            # Export the 4 second chunks as .wav files
